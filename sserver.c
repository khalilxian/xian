...
void* thread(void* arg)
{
    ...
    {	
		...
		if(ret==0){//退出线程，并删除该结点
			printf("ip:%s,port:%hu disconnect!\n",ipbuf,port);
			DelNode(head,sockfd);
			close(sockfd);
			pthread_exit(NULL);
		}
		if(ret==-1&&errno==EAGAIN){//接收超时
			printf("ip:%s,port:%hu timeout!\n",ipbuf,port);
			continue;
			//SendMsg(sockfd,"TimeOut",strlen("TimeOut"),CtoSWrong);//报告超时
			//close(sockfd);
			//pthread_exit(NULL);
		}
...
int main()
{
   ...
   /*将文件描述符和服务器地址绑定在一起*/
    ...
    /*声明listenfd处于监听状态，并且最多允许有LISMAX个客户端处于待连接状态*/
